  ...
  if (cart.length === 0 && step !== 3) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-[#f8fafc] font-sans text-center">
        ...
        <Link to="/shop" className="px-12 py-5 bg-[#0978CD] text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 active:scale-95">Return to Catalog</Link>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-[#f8fafc] font-sans text-center">
        ...
        <div className="bg-white p-12 rounded-[40px] border border-slate-100 mb-12 max-w-md w-full shadow-2xl shadow-indigo-100/30 overflow-hidden relative group">
          ...
          <p className="text-[11px] font-black text-[#0978CD] uppercase tracking-[0.4em] mb-4 relative z-10">Order Reference</p>
          <p className="text-4xl font-black text-slate-900 uppercase tracking-widest relative z-10 italic">#PTP-{orderId || 'PENDING'}</p>
        </div>
        <Link to="/" className="px-12 py-5 bg-[#0978CD] text-white font-black text-[11px] uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-100 active:scale-95">
          Return to Dashboard
        </Link>
      </div>
    );
      ...
      <div className="w-full mx-auto px-6 lg:px-16 py-10 md:py-14 mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
          <div className="space-y-4">
            <Link to="/cart" className="group inline-flex items-center gap-2 text-[11px] font-black text-slate-400 hover:text-[#0978CD] transition-all uppercase tracking-[0.25em]">
            ...
            <h1 className="text-4xl md:text-5xl font-black text-slate-900 leading-none tracking-tighter">
              Secure <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#0978CD] to-blue-500 italic">Checkout.</span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-6 bg-white px-8 py-5 rounded-[24px] border border-slate-100 shadow-sm">
            ...
            <div className={`h-[2px] w-10 rounded-full transition-all ${step >= 2 ? 'bg-[#0978CD]' : 'bg-slate-100'}`} />
            <div className={`h-12 w-12 rounded-xl flex items-center justify-center text-sm font-black transition-all border-2 ${step >= 2 ? 'bg-[#0978CD] border-[#0978CD] text-white shadow-lg' : 'border-slate-100 text-slate-200'}`}>2</div>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-12 xl:gap-24">
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-12 bg-white p-10 md:p-16 border border-slate-100 rounded-[40px] shadow-2xl shadow-indigo-100/20">
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12">
                <div className="space-y-8">
                  <div className="flex items-center gap-5 pb-6 border-b border-slate-50">
                    <div className="h-12 w-12 bg-indigo-50 text-[#0978CD] rounded-2xl flex items-center justify-center shadow-sm">
                  ...
                  <div className="space-y-3">
                    ...
                    <input required name="email" value={formData.email} onChange={handleInputChange} type="email" placeholder="Email address for specialized order updates" className="w-full h-16 px-8 bg-slate-50 border border-slate-100 rounded-2xl focus:bg-white focus:border-[#0978CD] outline-none text-sm font-bold text-slate-800 transition-all placeholder:text-slate-300" />
                  </div>
                </div>
                ...
                <div className="space-y-10">
                  <div className="flex flex-wrap items-center justify-between gap-6 pb-6 border-b border-slate-50">
                    ...
                    <button
                      ...
                      className="flex items-center gap-3 px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#0978CD] transition-all disabled:opacity-50 shadow-xl shadow-black/10 active:scale-95"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      ...
                      <input required name="city" value={formData.city} onChange={handleInputChange} placeholder="Pasadena, CA" className="w-full h-16 px-8 bg-slate-50 border border-slate-100 rounded-2xl focus:bg-white focus:border-[#0978CD] outline-none text-sm font-bold text-slate-800 transition-all placeholder:text-slate-300" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Postal Code</label>
                      <input required name="zipCode" value={formData.zipCode} onChange={handleInputChange} placeholder="Zip code" className="w-full h-16 px-8 bg-slate-50 border border-slate-100 rounded-2xl focus:bg-white focus:border-[#0978CD] outline-none text-sm font-bold text-slate-800 transition-all placeholder:text-slate-300" />
                    </div>
                  </div>
                ...
                <div className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    <div
                      ...
                      className={`p-10 rounded-[32px] border-2 transition-all cursor-pointer relative overflow-hidden group ${formData.paymentMethod === 'cod' ? 'border-[#0978CD] bg-indigo-50/30 shadow-2xl shadow-indigo-100' : 'border-slate-50 bg-slate-50/50 hover:border-slate-200'}`}
                    >
                      <div className="flex items-center justify-between mb-10">
                        <div className={`h-8 w-8 rounded-xl border-2 flex items-center justify-center transition-all ${formData.paymentMethod === 'cod' ? 'border-[#0978CD] bg-[#0978CD] text-white' : 'border-slate-200 bg-white'}`}>
                          {formData.paymentMethod === 'cod' && <CheckCircle2 size={16} strokeWidth={3} />}
                        </div>
                        <Truck size={40} className={formData.paymentMethod === 'cod' ? 'text-[#0978CD]' : 'text-slate-200'} strokeWidth={1.5} />
                      </div>
                    ...
                    <div
                      ...
                      className={`p-10 rounded-[32px] border-2 transition-all cursor-pointer relative overflow-hidden group ${formData.paymentMethod === 'paypal' ? 'border-[#0978CD] bg-indigo-50/30 shadow-2xl shadow-indigo-100' : 'border-slate-50 bg-slate-50/50 hover:border-slate-200'}`}
                    >
                      <div className="flex items-center justify-between mb-10">
                        <div className={`h-8 w-8 rounded-xl border-2 flex items-center justify-center transition-all ${formData.paymentMethod === 'paypal' ? 'border-[#0978CD] bg-[#0978CD] text-white' : 'border-slate-200 bg-white'}`}>
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="space-y-10 pt-10 mt-10 border-t border-slate-50">
                          <div className="p-10 bg-slate-900 rounded-[32px] text-white text-center border border-slate-800 relative overflow-hidden group">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-[#0978CD] rounded-full blur-[60px] opacity-20" />
            ...
            <div className="flex flex-col sm:flex-row gap-6 pt-12 border-t border-slate-50 mt-12">
              {step === 2 && (
                <button type="button" onClick={() => setStep(1)} className="px-12 h-16 bg-slate-50 border border-slate-100 text-slate-500 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-white hover:text-[#0978CD] transition-all active:scale-95 shadow-sm">Back to details</button>
              )}
              {formData.paymentMethod === 'cod' || step === 1 ? (
                <button
                  type="submit" disabled={loading}
                  className="flex-1 h-16 bg-[#0978CD] text-white rounded-2xl flex items-center justify-center gap-4 text-[11px] font-black uppercase tracking-[0.25em] hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-100 active:scale-95 disabled:opacity-50 group"
                >
            ...
            <div className="bg-white rounded-[40px] p-10 lg:p-14 border border-slate-100 sticky top-32 shadow-2xl shadow-indigo-100/30">
                ...
                {cart.map(item => (
                  <div key={item.id} className="flex gap-6 group items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-[13px] font-bold text-slate-800 truncate group-hover:text-[#0978CD] transition-colors leading-tight mb-2 uppercase tracking-tight">{item.name}</h4>
              ...
              <div className="space-y-8 border-t border-slate-50 pt-10">
                ...
                <div className="flex justify-between items-end pt-8 border-t border-slate-50">
                  ...
                  <span className="text-5xl font-black text-[#0978CD] leading-none tracking-tighter italic">${finalTotal.toLocaleString()}</span>
                </div>
              </div>
